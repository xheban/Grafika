...
// Static resources
unique_ptr<Mesh> AmmoCart::mesh;
unique_ptr<Texture> AmmoCart::texture;
unique_ptr<Shader> AmmoCart::shader;
...
bool AmmoCart::update(Scene &scene, float dt) {
...
void AmmoCart::explode(Scene &scene, vec3 explosionPosition, vec3 explosionScale) {
  // Generate explosion
...
void AmmoCart::render(Scene &scene) {
  shader->use();
