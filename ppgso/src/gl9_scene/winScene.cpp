...
WinScene::WinScene() {
    ...
    if (!texture) texture = make_unique<Texture>(image::loadBMP("winScene.bmp"));
    if (!mesh) mesh = make_unique<Mesh>("quad.obj");
}

bool WinScene::update(Scene &scene, float dt) {
...
void WinScene::render(Scene &scene) {
...
// shared resources
unique_ptr<Mesh> WinScene::mesh;
unique_ptr<Shader> WinScene::shader;
unique_ptr<Texture> WinScene::texture;
